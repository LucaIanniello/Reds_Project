...
def print_flags(flags, flags_def):
    logging.info(
        "Running training with hyperparameters: \n{}".format(
            pprint.pformat([f"{key}: {val}" for key, val in get_user_flags(flags, flags_def).items()])
        )
    )
...
def load_pickle(filename, output_dir):
    with open(os.path.join(output_dir, filename), "rb") as fin:
        obj = pickle.load(fin)
    return obj
